...
| `fred_withdraw_credit_check_zero_deferred_total` | counter | — | Credit checks that read an empty balance but deferred lease closure pending the `credit_check_zero_grace_period` window; a sustained high rate for a tenant that never closes suggests a chronically lagging chain node or a mistuned grace period (ENG-591) |
