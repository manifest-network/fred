...
`make all` builds six binaries; `providerd`, `docker-backend`, the offline `placement-preflight` inspector/preparer/initializer, and the offline `placement-repair` tool are released. `placement-preflight` is read-only by default; only explicit `-prepare` or `-initialize-fresh` modes write. `mock-backend` is for dev/E2E, and **`k3s-backend` is a non-functional scaffold (ENG-133)** — it serves the backend contract, but every provision returns `failed / "not implemented"` (`internal/backend/k3s/provision_stub.go`). `cmd/loadtest` and `cmd/lease-token` have no make target — `go run ./cmd/<name>`.
...
- **Routing is by exact SKU UUID** (`internal/backend/router.go`); on multiple matches new provisions go to the least-loaded. An **empty SKU list matches nothing** — designate a fallback with `default: true`. The **placement store** (bbolt, `internal/provisioner/placement`) maps `lease → backend` so reads and retained-lease restores reach the source machine even after close; it is mandatory in production, and capability-safe constructors reject nil or typed-nil authority. Deprovision resolves the backend *positively* (placement, then the active operation in `operation.Registry`), never by SKU — a guessed deprovision reports success while stranding the volume.
- **Lease actor model (docker-backend).** One goroutine per lease serializes state mutation; the SM lives in the substrate-agnostic `internal/backend/shared/leasesm`. The actor holds no backend pointer and reaches the live `ProvisionState` projection only through `LeaseProvisionStore`; the ENG-229 invariant block on that interface documents the one bounded worker pre-publication needed for deprovision preemption. `Backend.provisions` guarded by `provisionsMu` is the deliberate projection substrate, **not** durable settlement authority: new request mutations are typed actor commands backed by their exact journal claims, never direct map mutation.
...
- **Startup order is load-bearing** (API server → provision manager, blocking on `Running()` → launch non-blocking maintenance recovery → one-shot `WithdrawOnce` → startup reconcile → everything else). Durable maintenance claims are rehydrated during construction, but their first network retry runs in the background so one unavailable backend cannot hold fleet startup. Reconciliation triggers backend callbacks, so callback admission and the acknowledgment lanes must be live; chain and payload events still require the Watermill handlers.
...
- **`recover()` is for goroutine and foreign-code boundaries** — actor handlers, long-lived loops, per-item workers (reconciler per-lease/per-orphan, restore), and the coordinator-owned invocation of a pluggable backend or best-effort observer. At a synchronous backend boundary, a panic is an observed ambiguous call result: preserve the write-ahead attempt and never let the caller inject a replacement outcome. Ordinary internal call paths return errors. The `sdkConfigOnce` block in `cmd/providerd/main.go` retains a narrow panic-class guard. Chain event delivery instead holds the subscriber read lock through nonblocking sends; unsubscribe and close hold its write lock, so channel closure cannot race a send. `cmd/providerd` also wraps its long-lived components in a local `safeGo()` (panic → error on `errChan`); recover sites should log enough bounded context and bump the matching `*_panics_total` counter when one exists.
- **No test-only hooks in production code** — no nil-in-prod fields, flags, or injection points; use consumer-side interfaces, an in-package `export_test.go`, or split the function so a white-box test drives the pieces in order (ENG-765 did the last for `runStubProvisioner`). Enforced by `internal/testutil/no_test_only_code_test.go` (walks top-level decls **and** struct fields; build tags do not hide from it). Its blind spot is a field production writes but only tests read — no phrase can see that. Chain mocks live in `internal/chain/chaintest/`, shared fixtures in `internal/testutil/fixtures.go`.
...
- **A new backend failure class needs a typed transport verdict before it can change durable or tenant-visible state.** `IsSuccessful` (`internal/backend/client.go`) still decides whether it trips the circuit breaker, but package-owned `backend.Invoke*` functions are the causal choke points: only the exact identity-bound HTTP client can mint a closed refusal/no-dispatch outcome. Placement and application layers project those opaque results; never recover authority with `errors.Is` over an arbitrary backend error tree. The tenant-facing `backend.Reason` + curated message is **authored at the failure source** (`internal/backend/reason.go`), never derived from `err.Error()` — an unmapped path silently degrades to `Unknown` (ENG-508).
...
- **Retention and cleanup gates fail open toward keeping bytes** — refusing to retain and reaping an orphan both destroy data, so collapse uncertainty toward "keep" and count it in a metric. This does not require readiness/admission to open when the kept footprint or its quota authority cannot be accounted safely; admission gates still fail closed. Either way, test the error branch.
