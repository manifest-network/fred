...
- **`recover()` is for goroutine and foreign-code boundaries** — actor handlers, long-lived loops, per-item workers (reconciler per-lease/per-orphan, restore), and the coordinator-owned invocation of a pluggable backend or best-effort observer. At a synchronous backend boundary, a panic is an observed ambiguous call result: preserve the write-ahead attempt and never let the caller inject a replacement outcome. Ordinary internal call paths return errors. The remaining narrow panic-class guards are `chain.trySend` (send on a concurrently-closed subscriber channel) and the `sdkConfigOnce` block in `cmd/providerd/main.go`. `cmd/providerd` also wraps its long-lived components in a local `safeGo()` (panic → error on `errChan`); recover sites should log enough bounded context and bump the matching `*_panics_total` counter when one exists.
