...
>
> **Downgrading after ENG-548:** once a volume has been provisioned with an inode limit (`ihard`), rolling back to a pre-ENG-548 binary — whose `Destroy` clears only `bhard`/`bsoft` — leaves `ihard` in place, silently reintroducing the ENG-459 stuck-entry leak. `volume_quota_clear_failed_total` will **not** fire in this case, since the old binary's partial clear still reports success, so the alert-driven runbook above won't catch the regression. The remedy is the same ENG-459 operator sweep, using the four-limit clear command above.
