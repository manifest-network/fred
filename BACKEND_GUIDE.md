...
The one documented exception is a **bare** response — no body at all. `/restore` specifies a bodiless `422` as "no retained data", so a backend without retention support can answer with nothing. That is "no body", not "any body fred cannot read": a `422` carrying an unparseable body is treated as a contract violation, not as the bare form.
