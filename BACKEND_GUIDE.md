...
The one documented exception is a **bare** response — no body at all. `/restore` specifies a bodiless `422` as "no retained data", so a backend without retention support can answer with nothing. That exemption is for an **empty** body only. Any other body must be the envelope with a non-empty `error`, including on `409`/`422`: an unparseable body, and a body that is valid JSON but omits `error` (`{}`, `null`, `{"message": "..."}`, or even `{"code": "..."}`), are contract violations, not the bare form. A discriminator alone does not substitute for `error` — send both.
