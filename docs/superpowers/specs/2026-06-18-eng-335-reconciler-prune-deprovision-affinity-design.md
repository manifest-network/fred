...
- `Set` stamps `SetAt = s.now()`. `SetBatch` (reconciler additive sync) **preserves** an existing record's `SetAt` and only stamps `s.now()` for entries it creates — `SetAt` means "when we first learned this placement", and must not be reset every sweep (analogous to an immutable `creationTimestamp`).
