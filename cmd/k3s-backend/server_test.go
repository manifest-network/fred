...
// newTestHandler creates a Handler backed by a nil backendService. Used by
// middleware/validation tests that exercise paths short-circuiting before any
// backend call. Valid-sig pass-through is asserted via assert.Panics — the
...
func (m *mockBackend) ReconcileCustomDomain(ctx context.Context, leaseUUID string, items []backend.LeaseItem) error {
	if m.ReconcileCustomDomainFunc == nil {
		...
		// reconciler hits it on every healthy lease.
		return nil
	}
...
func TestProvision_RequiresAuth(t *testing.T) {
	...
	t.Run("valid signature passes auth", func(t *testing.T) {
		req := signedPostRequest("/provision", body)
		w := httptest.NewRecorder()
		// Nil backend panics on Provision call after middleware accepts.
...
func TestGetInfo_RequiresAuth(t *testing.T) {
	...
	t.Run("valid signature passes auth", func(t *testing.T) {
		req := signedGetRequest("/info/lease-1")
		w := httptest.NewRecorder()
		assert.Panics(t, func() { handler.ServeHTTP(w, req) })
	})
}
...
func TestGetProvision_PassesAuth(t *testing.T) {
	handler := newTestHandler()
	req := signedGetRequest("/provisions/lease-1")
	w := httptest.NewRecorder()
	assert.Panics(t, func() { handler.ServeHTTP(w, req) })
}
...
func TestGetReleases_PassesAuth(t *testing.T) {
	handler := newTestHandler()
	req := signedGetRequest("/releases/lease-1")
	w := httptest.NewRecorder()
	assert.Panics(t, func() { handler.ServeHTTP(w, req) })
}
...
func TestHealth_NoAuthRequired(t *testing.T) {
	...
	w := httptest.NewRecorder()
	// Passes without auth, then panics on nil-backend Health call.
	assert.Panics(t, func() { handler.ServeHTTP(w, req) })
}
...
func TestMetrics_NoAuthRequired(t *testing.T) {
	...
	w := httptest.NewRecorder()
	// Metrics is served by promhttp.Handler() — no auth, no backend call.
	handler.ServeHTTP(w, req)
	assert.Equal(t, http.StatusOK, w.Code)
}

// --- Validation gate (tail param, missing fields) ------------------------
	...
	t.Run("success with default tail returns 200", func(t *testing.T) {
		mb := &mockBackend{
			GetLogsFunc: func(_ context.Context, leaseUUID string, tail int) (map[string]string, error) {
				...
				return map[string]string{"0": "log output"}, nil
			},
		}
		w := httptest.NewRecorder()
		...
		require.NoError(t, json.Unmarshal(w.Body.Bytes(), &got))
		assert.Equal(t, "log output", got["0"])
	})
	...
	t.Run("success with custom tail", func(t *testing.T) {
		mb := &mockBackend{
			GetLogsFunc: func(_ context.Context, _ string, tail int) (map[string]string, error) {
				...
				return map[string]string{"0": "ok"}, nil
			},
		}
		w := httptest.NewRecorder()
...
func TestHandleListProvisions_Filtered(t *testing.T) {
	...
	t.Run("unknown leases serialize as []", func(t *testing.T) {
		mb := &mockBackend{
		...
		assert.Contains(t, w.Body.String(), `"provisions":[]`,
			"empty filtered result must serialize as [] not null")
	})
...
func TestReconcileCustomDomain_DispatchesToBackendAndReturns204(t *testing.T) {
	// Happy path: signed valid body reaches the backend and the handler
	// returns 204 No Content. Recording mockBackend verifies dispatched
	// arguments. No mutex needed — handler.ServeHTTP is synchronous, so
	// the callback runs in the calling goroutine and assertions read the
	// captured state after the synchronous return.
	var (
	...
	mb := &mockBackend{
		ReconcileCustomDomainFunc: func(_ context.Context, leaseUUID string, items []backend.LeaseItem) error {
			invocationCount++
...
func TestReconcileCustomDomain_BackendErrorReturns500(t *testing.T) {
	mb := &mockBackend{
		ReconcileCustomDomainFunc: func(context.Context, string, []backend.LeaseItem) error {
...
func TestReconcileCustomDomain_NotProvisionedReturns404(t *testing.T) {
	// ErrNotProvisioned surfaces as 404 so HTTPClient can map it back to
	// the typed error — providerd-side circuit breaker exempts it from the
	// trip count (matches docker rationale).
	mb := &mockBackend{
		ReconcileCustomDomainFunc: func(context.Context, string, []backend.LeaseItem) error {
			return backend.ErrNotProvisioned
		},
...
func TestHandleHealth(t *testing.T) {
	...
	t.Run("unhealthy returns 503", func(t *testing.T) {
		mb := &mockBackend{
			// Mirrors T3's "k3s API unreachable: %w" wrap; the handler
			// surfaces the error message in the 503 body.
			HealthFunc: func(context.Context) error { return fmt.Errorf("k3s API unreachable: i/o timeout") },
		}
		handler := newMockHandler(mb)
		...
		assert.Equal(t, http.StatusServiceUnavailable, w.Code)
		assert.Contains(t, w.Body.String(), "k3s API unreachable")
	})
}
...
func TestHandleStats(t *testing.T) {
	...
	// Available* fields are computed (Total - Allocated).
