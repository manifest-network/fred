...
// TestParseFlags_Version is the ENG-255 test: k3s-backend must answer
// `--version` by printing the build-injected version and signalling the
// caller to exit BEFORE any config is loaded — matching providerd and
// docker-backend (ENG-254). An operator can query the version without a
...
func TestParseFlags_Version(t *testing.T) {
	t.Run("--version prints version and signals exit", func(t *testing.T) {
		...
		assert.Equal(t, "k3s-backend version "+version+"\n", out.String())
	})
	...
	t.Run("default config path is preserved when no flags are passed", func(t *testing.T) {
		...
		require.NoError(t, err)
		assert.False(t, showVersion)
		assert.Equal(t, "config.k3s.yaml", cfgPath)
	})
}
