...
func TestUpdateLeaseReturnsCuratedBackendRefusalDetailAndReplaysReceipt(t *testing.T) {
	t.Parallel()
	const (
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case backendidentity.BoundPathPrefix + storageID.String() + "/update":
			updateCalls.Add(1)
	...
	for attempt := range 2 {
		request := httptest.NewRequest(http.MethodPost, "/v1/leases/"+leaseUUID+"/update",
			strings.NewReader(`{"payload":"bWFuaWZlc3Q="}`))
	...
	assert.Equal(t, int32(1), updateCalls.Load(), "the real service must replay the durable refusal without another backend update")
	storedPayload, err := payloadStore.Get(leaseUUID)
	require.NoError(t, err)
	assert.Equal(t, []byte("previously accepted manifest"), storedPayload, "a refused update must preserve the accepted manifest")
}
