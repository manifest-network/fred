...
func TestCanonicalWireContract(t *testing.T) {
	...
	assert.Equal(t, "test_8b0ceb413e8c0f0bd2d83906",
		id.DiagnosticFingerprint("test-id-v1", "test_"))
	text, err := id.EncodeText(invalid)
	require.NoError(t, err)
	assert.Equal(t, canonical, string(text))

	for _, value := range []string{
		"", "550E8400-E29B-41D4-A716-446655440000", uuid.Nil.String(),
		...
		if value != "" {
			assert.NotContains(t, err.Error(), value,
				"validation errors must not disclose typed authority")
		}
	}
	...
	assert.Equal(t, "invalid", zero.DiagnosticFingerprint("test-id-v1", "test_"))
	_, err = zero.EncodeText(invalid)
...
func TestQueryHelpersPreserveTypedExactlyOnceSemantics(t *testing.T) {
	...
	parse := func(value string) (Value, error) { return Parse(value, invalid) }
	id, present, err := ParseQuery(url.Values{}, parameter, ambiguous, parse)
	require.NoError(t, err)
	assert.False(t, present)
	assert.False(t, id.Valid())

	id, present, err = ParseQuery(url.Values{parameter: {canonical}}, parameter, ambiguous, parse)
	require.NoError(t, err)
