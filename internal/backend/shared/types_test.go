...
func TestSKUProfile_Validate(t *testing.T) {
	tests := []struct {
		...
		{name: "max valid burst", profile: SKUProfile{CPUCores: 1, MemoryMB: 512, BandwidthMbps: 100, BurstKB: 1_048_576}},
		{name: "max valid latency", profile: SKUProfile{CPUCores: 1, MemoryMB: 512, BandwidthMbps: 100, LatencyMs: 10_000}},
		{name: "burst without bandwidth", profile: SKUProfile{CPUCores: 1, MemoryMB: 512, BurstKB: 64}, wantErr: "burst_kb and latency_ms have no effect when bandwidth_mbps is 0"},
		{name: "latency without bandwidth", profile: SKUProfile{CPUCores: 1, MemoryMB: 512, LatencyMs: 50}, wantErr: "burst_kb and latency_ms have no effect when bandwidth_mbps is 0"},
