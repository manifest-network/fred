...
func TestCloseTerminalAuthorityPreservesUnsettledLaunch(t *testing.T) {
	for _, retain := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			require.True(t, IsLifecyclePending(pending), "durable launch debt is an availability observation, not a server failure")
			require.False(t, pending.RetryableNow())
			current, found, err := settlement.GetCloseIntent(claim.LeaseUUID())
			require.NoError(t, err)
			...
			acquired, err := coordinator.WithLease(t.Context(), claim.LeaseUUID(), func(scope LeaseRecoveryScope) error {
				recovered, recoverErr := settlement.RecoverCloseExecution(t.Context(), scope, current)
				require.NoError(t, recoverErr)
				pending, ok := recovered.(CloseExecutionPending)
				require.True(t, ok)
				require.True(t, IsLifecyclePending(pending), "recovery must preserve the same typed launch-debt observation")
				require.False(t, pending.RetryableNow())
				return nil
			})
			require.NoError(t, err)
