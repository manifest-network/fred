...
// An owner of "" asserts no ownership at all, so ANY claim refuses it; that is how the
// collectors ask "is this spoken for?" without a second mechanism.
func (c *volumeClaims) mayDestroy(name, owner string) (volumeClaim, bool) {
	claim, claimed := c.owner(name)
	if !claimed {
		return volumeClaim{}, true
	}
	return claim, owner != "" && claim.owner == owner
}
...
func (o *volumeOp) destroy(ctx context.Context, site string, names ...string) destroyReport {
	...
	for _, name := range names {
		...
		if err := sink.Destroy(ctx, name); err != nil {
