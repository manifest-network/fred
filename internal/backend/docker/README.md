...
A tombstone is a *name-keyed* scheduled destroy — it carries strings, not proof of ownership — and while a restore is in flight the original lease's data wears the new lease's canonical name, so the finalizer re-checks ownership **at destroy time**, not just when the record was written (ENG-659). A name an in-flight restore has adopted is skipped and the record is kept, so the next sweep retries once that restore's **rollback** re-quarantines the volume back into the `fred-retained-` namespace. Rollback is the only resolution the sweep can reach: a lease that carries a give-up tombstone has already had its containers confirmed gone (the give-up arm is unreachable until teardown succeeds) and its provision deleted, while `reconcileRestoring`'s finalize arm requires a live Ready provision — so it can only take the rollback arm. That asymmetry is load-bearing, because a commit would delete the retention record while leaving the volume at the very canonical name the tombstone carries, handing the next sweep a claim-free name for live data. An unreadable retention store means nothing is destroyed that pass. Both are counted on `fred_docker_backend_retention_reap_skips_total{reason}` and are deliberately **not** counted as leaks — nothing is abandoned, and the footprint keeps counting while the record stays `reaping`.
...
- `fred_docker_backend_retention_sweep_total{outcome}` — one increment per periodic retention-sweep pass, `success` or `error`. The sum across outcomes is a liveness heartbeat (it advances every tick regardless of result); `{outcome="error"}` means a sweep stage failed, which in practice means the retention store cannot be enumerated. Every stage runs on every pass and the stage errors are joined, so the log line names all of them rather than only the first.
